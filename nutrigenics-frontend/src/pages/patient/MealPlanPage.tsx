...
import { Calendar, Plus, Flame, Trash2, Search, Utensils, Egg, Coffee, Moon, ChefHat, ArrowUpRight, History } from 'lucide-react';
...
export default function MealPlanPage() {
  ...
  const [sheetMode, setSheetMode] = useState<'add_item' | 'new_section'>('add_item');
  ...
  useEffect(() => {
    if (sheetMode === 'add_item' && isSheetOpen) {
      searchRecipes();
    }
  }, [searchQuery, isSheetOpen, sheetMode]);
  ...
  const handlePortionConfirm = async (portionData: PortionData) => {
    try {
      ...
      if (selectedRecipe) {
        ...
        // Calculate nutrients from recipe to ensure they are saved
    ...
    <>
      <div className="flex flex-col gap-8 pb-20">
        ...
        <div className="flex flex-col gap-6">
          ...
          <Card className="rounded-lg p-6 shadow-sm border-border bg-card">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              ...
              <div className="flex flex-col items-center justify-center gap-2">
                ...
                <span className="text-sm font-bold text-muted-foreground uppercase tracking-wider">Calories</span>
              </div>
              ...
              <div className="flex flex-col items-center justify-center gap-2">
                <CircularProgress
                  ...
                  color="text-blue-500"
                />
                <span className="text-sm font-bold text-muted-foreground uppercase tracking-wider">Carbs</span>
              </div>
                              ...
                              <div key={item.id} className="flex items-center gap-4 p-3 rounded-2xl bg-muted/30 border border-transparent hover:border-border transition-colors group">
                                  ...
                                  {item.image ? (
                                    <img
                                      src={item.image.startsWith('http')
                                        ? item.image
                                        : `${import.meta.env.VITE_API_URL || 'http://localhost:8003'}${item.image}`}
                                ...
                                >
                                  ...
                                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground mt-1">
                                    ...
                                    <span>{item.protein?.toFixed(1) || 0}g Pro</span>
                                    <span className="w-1 h-1 rounded-full bg-border" />
                                    <span>{item.carbs?.toFixed(1) || 0}g Carb</span>
                                    <span className="w-1 h-1 rounded-full bg-border" />
                            ...
                            {p.image ? (
                              <img
                                src={p.image.startsWith('http')
                                  ? p.image
                                  : `${import.meta.env.VITE_API_URL || 'http://localhost:8003'}${p.image}`}
                                className="w-full h-full object-cover"
                                alt=""
                              />
                            ) : (
      ...
      {/* REUSED: Add Item Sheet */}
      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        <SheetContent side="right" className="w-[100%] sm:w-[500px] p-0 overflow-hidden bg-background">
          <div className="h-full flex flex-col">
            <SheetHeader className="p-6 pb-2">
              <SheetTitle className="text-2xl font-bold">
                {sheetMode === 'add_item' ? `Add to ${activeSection}` : 'Create New Section'}
              </SheetTitle>
              <SheetDescription>
                {sheetMode === 'add_item' ? 'Choose a recipe or log a custom meal.' : 'Add a custom meal category to your day.'}
              </SheetDescription>
            </SheetHeader>
            ...
            {sheetMode === 'add_item' ? (
              <Tabs defaultValue="catalog" className="flex-1 flex flex-col">
                ...
                <TabsContent value="custom" className="flex-1 px-6 pb-6 mt-0 overflow-y-auto">
