...
export function PatientChatDialog({ open, onOpenChange, patient }: PatientChatDialogProps) {
    const { user } = useAuth(); // Dietitian
    ...
    useEffect(() => {
        if (!open || !patient?.user?.id) return;
        ...
        const init = async () => {
            try {
                // Fetch Messages
                const msgs = await chatService.getMessages(patient.user!.id);
                setMessages(msgs);
                setIsLoading(false);
                ...
                intervalId = setInterval(async () => {
                    try {
                        const newMsgs = await chatService.getMessages(patient.user!.id);
                        setMessages(prev => {
    ...
    const handleSend = async (e: React.FormEvent) => {
        e.preventDefault();
        if ((!newMessage.trim() && !selectedFile) || !patient?.user?.id || !user) return;
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md md:max-w-2xl lg:max-w-3xl h-[85vh] p-0 gap-0 overflow-hidden bg-[#F8FAFC] border-0 shadow-2xl flex flex-col">
                ...
                <DialogHeader className="h-[72px] px-6 flex flex-row items-center justify-between border-b border-gray-100 bg-white/80 backdrop-blur-md z-40 shrink-0">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => onOpenChange(false)} className="rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-100" aria-label="Close chat">
                        <X className="w-5 h-5" />
                    </Button>
                </DialogHeader>
                ...
                <div className="flex-1 overflow-y-auto px-4 md:px-6 py-6 scroll-smooth custom-scrollbar relative bg-[#FAFAFA]">
                    ...
                    <div className="max-w-3xl mx-auto space-y-6">
                        ...
                        <div className="flex justify-center mb-6">
                            <Badge variant="outline" className="bg-slate-50 text-slate-400 border-slate-200 font-normal px-3 py-1 text-xs uppercase tracking-widest">
                                Today
                            </Badge>
                        </div>
                        ...
                        {isLoading && messages.length === 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-20 opacity-50 select-none">
                                ...
                                <p className="text-sm text-slate-500 mt-1">Start a conversation with {patient.user?.first_name}</p>
                            </div>
                        ) : (
                            messages.map((msg) => {
                                const isMyMessage = msg.sender === user?.id; // Dietitian is current user
                ...
                <div className="bg-white p-4 shrink-0 relative z-50">
                    <div className="max-w-3xl mx-auto relative">
                        {/* Preview */}
                        {selectedFile && (
                            <div className="absolute -top-20 left-0 bg-white p-2 rounded-xl shadow-lg border border-slate-100 flex items-center gap-3 animate-in fade-in slide-in-from-bottom-2 z-10">
                                {previewUrl ? (
                                ...
                                <div className="flex-1 min-w-0 pr-2">
                                    <p className="text-xs font-semibold text-slate-900 truncate max-w-[150px]">{selectedFile.name}</p>
                                    <p className="text-xs text-slate-500">{(selectedFile.size / 1024).toFixed(0)} KB</p>
                                </div>
                                <button type="button" onClick={clearFile} className="p-1 hover:bg-slate-100 rounded-full text-slate-400 hover:text-red-500 transition-colors" aria-label="Remove selected file">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        )}
                        ...
                        <form onSubmit={handleSend} className="relative group">
                            <div className="bg-slate-50 p-1.5 rounded-[2rem] border border-slate-200 flex items-center gap-2 focus-within:bg-white focus-within:shadow-md focus-within:border-emerald-200 transition-all duration-200">
                                ...
                                <Button
                                    ...
                                    className="h-10 w-10 rounded-full text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors shrink-0"
                                    title="Attach file"
                                ...
                                <Input
                                    ...
                                    className="flex-1 bg-transparent border-0 focus-visible:ring-0 shadow-none px-2 text-base placeholder:text-slate-400 h-11"
                                    disabled={isSending}
                                />
                        ...
                        <div className="text-center mt-2">
                            <p className="text-xs font-medium text-slate-400 flex items-center justify-center gap-1.5 opacity-60">
