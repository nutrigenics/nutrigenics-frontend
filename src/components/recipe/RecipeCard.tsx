...
export function RecipeCard({ recipe, className, variant = 'default', onClick, actions }: RecipeCardProps) {
    ...
    let imageUrl = '/illustrations/recipe-placeholder.png';
    if (recipe && recipe.recipe_image && recipe.recipe_image !== 'undefined') {
        if (recipe.recipe_image.startsWith('http')) {
            imageUrl = recipe.recipe_image;
        } else if (recipe.recipe_image.startsWith('/')) {
            imageUrl = `${API_BASE_URL}${recipe.recipe_image}`;
        } else {
            imageUrl = `${API_BASE_URL}/media/${recipe.recipe_image}`;
        }
    }
    ...
    const DefaultCardContent = (
        <div className="rounded-2xl flex flex-col gap-4 p-3">
            {/* Recipe Image */}
            <div className="w-full aspect-[3/2] overflow-hidden border rounded-lg relative">
                <img
                    src={imageUrl}
                    alt={recipe.recipe_name}
                ...
                {!onClick && (
                    <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
                        <div className="bg-background/80 backdrop-blur-md p-2.5 rounded-lg shadow-lg border border-border">
    ...
    return (
        <Card className={cn(
            'overflow-hidden border-border bg-card text-card-foreground shadow-premium hover:shadow-premium-lg transition-all duration-300',
